...
Bench is a small, responsive chatbot built for the Career141 technical assessment. It uses Next.js for the user interface and server-side API route, and Google Gemini for AI responses.

## Highlights

- Clean chat interface with starter prompts, loading feedback, automatic scrolling, and friendly errors.
- A server-side API route at `pages/api/chat.js`; the browser never calls Gemini directly.
...
pages/
  ...
  api/chat.js       # Server-only route that calls Gemini
...
- Node.js 18 or newer
- A Gemini API key created in [Google AI Studio](https://aistudio.google.com/app/apikey)
...
Important: use `.env.local` only for your real key. Do not put it in `pages/index.js`, any `NEXT_PUBLIC_*` variable, a screenshot, or a Git commit. The server route reads it using `process.env.GEMINI_API_KEY`, so it is not included in the browser bundle.
...
If the chat shows a missing-key error, confirm that `.env.local` is in the repository root, the variable is named exactly `GEMINI_API_KEY`, and restart `npm run dev` after editing the file.
...
```text
Browser -> POST /api/chat -> Gemini API
                 |
...
- `pages/api/chat.js` is the only file that reads `GEMINI_API_KEY`.
...
The first command should show that `.env.local` is ignored. The second should not list `.env.local`. If a key is ever committed, revoke it in Google AI Studio, generate a replacement, update the deployment environment variable, and remove it from Git history before making the repository public.
...
- [ ] The Git history uses small, meaningful commits, for example:
  ...
  feat: add secure Gemini API route
